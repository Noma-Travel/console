export default function ChatWidgetTransient({ key, item }: { key: string; item: { _out: { content: string | number | any } } }) {
  ...
  return (
        ...
        <span key={key} className="flex flex-col mb-4 w-full max-w-[75%] mx-auto">
            <div className="text-xs text-center mt-0 mb-0 text-muted-foreground/30">
            {item?._out?.content ? (
